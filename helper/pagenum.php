<?php 
	function generate_page_number($data_per_page, $total_data, $halaman_ke=1, $serial){
		//asumsi tampilan halaman per halaman ada 3
		$jum_hal=3;			
		$max = ceil($total_data / $data_per_page);
		echo "<ul>";
			if($halaman_ke>1){
				echo "<li><a href='?page=".($halaman_ke-1)."&serial=$serial'>Previous</a>&nbsp;</li>";
			}
		if ($jum_hal>=$max) {
			for($hal = 1; $hal<=$max; $hal++){
				...
				if($halaman_ke==$hal){
					...
					echo "<a href='?page=$hal&serial=$serial'>$hal</a>";
				}
				echo " &nbsp;</li>";
			}
		}
		else
		{
			...
			foreach ($arr_halaman as $value) {
				...
				if($halaman_ke==$value){
					...
					echo "<a href='?page=$value'&serial=$serial>$value</a>";
				}
				echo " &nbsp;</li>";
			}
		}
		if($halaman_ke<$max){
			...
			echo "<li><a href='?page=".$halaman_selanjutnya."&serial=$serial'>Next</a>&nbsp;</li>";
		}
		echo "</ul>";
	}
